...
def collect(
    ...
    for i, sec in enumerate(time_values):
        if filtered.empty:
            ...
            window = filtered[
                filtered["seconds_remaining"].between(sec - _TIME_WINDOW_S, sec + _TIME_WINDOW_S)
            ]
            ...
            wp_foul    = float(foul_outcomes.mean())    if len(foul_outcomes)    > 0 else _DEFAULT_WIN_RATE
            wp_no_foul = float(no_foul_outcomes.mean()) if len(no_foul_outcomes) > 0 else _DEFAULT_WIN_RATE
    ...
    with open(meta_path, "w") as fh:
        json.dump({"time_values": time_values, "fg3_pct_values": fg3_values}, fh, indent=2)
...
def plot(
    ...
    sns.heatmap(
        ...
        cmap=PALETTE, center=0.0, annot=True, fmt=".1f",
...
def generate_doc(
    ...
    def _cell(sec: int, fg3: float):
        ...
        return float(wp_foul_grid[i, j]), float(wp_no_foul_grid[i, j]), float(gain_grid[i, j])
...
"""
    ...
    content = _TEMPLATE.format(
        fg3_min=min(fg3_values), fg3_max=max(fg3_values),
        min_gain_pp=float(gain_grid.min() * 100),
        max_gain_pp=float(gain_grid.max() * 100),
        key_findings=key_findings, conclusion=conclusion,
        wp_foul_8_28=_fmt_ev(wf_8_28), wp_no_foul_8_28=_fmt_ev(wn_8_28),
        wp_gain_8_28=_fmt_gain(wg_8_28, pp=True),
        wp_foul_8_36=_fmt_ev(wf_8_36), wp_no_foul_8_36=_fmt_ev(wn_8_36),
        wp_gain_8_36=_fmt_gain(wg_8_36, pp=True),
        wp_foul_8_44=_fmt_ev(wf_8_44), wp_no_foul_8_44=_fmt_ev(wn_8_44),
        wp_gain_8_44=_fmt_gain(wg_8_44, pp=True),
        wp_foul_4_36=_fmt_ev(wf_4_36), wp_no_foul_4_36=_fmt_ev(wn_4_36),
